...
| `provenance` | device, card, CUDA runtime, compute capability, driver version, SM clock during the counted steps (`sm_clock_mhz_busy`: `min`, `median`, `max` and `sample_count` of `nvidia-smi` samples taken every 100 ms, cut to the post-warm-up window; memory is never read this way. With no sample in the window the numbers are `null`, and `unavailable_reason` says `"not a CUDA run"` or `"no nvidia-smi tool found"` when nothing could be sampled at all), platform, Python, package versions (torch, transformers, peft, trl, bitsandbytes, accelerate), dtype, optimizer, seed, data seed |
